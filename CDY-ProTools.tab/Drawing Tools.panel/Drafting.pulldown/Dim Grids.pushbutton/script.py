...
# -- Get grids (pre-selected or pick) ---------------------------------------
def get_grids():
...
if any(not (direction(g).IsAlmostEqualTo(base_dir) or direction(g).IsAlmostEqualTo(base_dir.Negate()))
       for g in grids[1:]):
    forms.alert("Grids must be parallel.")
    raise SystemExit

# Sort grids along their perpendicular axis
view_normal = view.ViewDirection.Normalize()
perp_dir    = direction(grids[0]).CrossProduct(view_normal).Normalize()
grids       = sorted(grids, key=lambda g: curve_in_view(g).Evaluate(0.5, True).DotProduct(perp_dir))
...
# Build dimension line through the picked point
