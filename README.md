To Install:

1.	If you have previously installed pyRevit, please uninstall and **ENSURE** you have the latest version of Revit (2022.1.7, 2023.1.5, 2024.3, 2025.4  – Tried and Tested).
2.	If Revit is open, close entirely.
3.	Install pyRevit_5.0.0.25034_signed.exe.
4.	Copy "extensions.json", from 'Installation Files' and paste to (overwriting existing file): %AppData%\pyRevit-Master\extensions. (AppData is hidden in files - ensure Hidden files are enabled in the "View" tab.
5.	Copy "Dynamo Revit" Folder, 'Installation Files' and paste to (overwriting existing file): %AppData%\Dynamo.
6.	Create a new project within Revit (this project won't be saved, therefore any name / template can be used).
7.	Navigate to pyRevit within the ribbon. On the left hand side click the pyRevit drop down > Extensions and install extension "BIMTools". Save to default location on C:\ Drive
8.	Close Revit entirely.
9.	Copy “pyRevit_config”, 'Installation Files' and paste to (overwriting existing file): %AppData%\pyRevit.
10.	Open a project and you should now have access to DevTools.
